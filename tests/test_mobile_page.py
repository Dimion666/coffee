...
class MobilePageTests(unittest.TestCase):
    ...
    def test_mobile_page_is_available(self) -> None:
        ...
        self.assertIn("cameraButton", response.text)
        self.assertIn("galleryButton", response.text)
        self.assertIn('id="cameraInput"', response.text)
        self.assertIn('capture="environment"', response.text)
        self.assertIn('id="galleryInput"', response.text)
